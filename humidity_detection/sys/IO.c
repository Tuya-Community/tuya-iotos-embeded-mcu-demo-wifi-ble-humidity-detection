...
void TASK_SHT21(void)
{
	uint8_t error=0;   
	nt16 sRH;
	unsigned long humidityRH;
	
	error += SHT2x_MeasureHM(HUMIDITY, &sRH);
  air_humi.now   = SHT2x_CalcRH(sRH.u16);
  humidityRH =(unsigned long)(air_humi.now*10);
	mcu_dp_value_update(DPID_HUMIDITY_VALUE,humidityRH);
}
